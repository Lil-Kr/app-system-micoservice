import { RoleAdminTableType, RoleTableType } from '@/types/apis/sys/role/roleType'
...
interface RoleAclState {
  ...
  // 存储穿梭框左边的数据(sourse)
  roleUserList: RoleAdminTableType[]
  setRoleUserList: (roleUserList: RoleAdminTableType[]) => void
}
...
const useRoleAclStore = create<RoleAclState>()((set, get) => ({
  ...
  setRoleUserList: (roleUserList: RoleAdminTableType[]) =>
    set(state => {
      return {
        ...state,
        roleUserList
