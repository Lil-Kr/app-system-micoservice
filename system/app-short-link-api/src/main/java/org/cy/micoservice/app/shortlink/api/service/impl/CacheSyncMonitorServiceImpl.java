...
public class CacheSyncMonitorServiceImpl implements CacheSyncMonitorService {
  ...
  public void monitorStreamStatus() {
    try {
      // 获取Stream信息
      var streamInfo = redisTemplate.opsForStream().info(REMOTE_STREAM_KEY);
      ...
      log.info("消费者组: {}", REMOTE_CONSUMER_SYNC_GROUP);
      ...
      // 检查消费者组状态
      var groupInfo = redisTemplate.opsForStream().groups(REMOTE_STREAM_KEY);
  ...
  private void emergencyCleanup() {
    try {
      ...
      Long trimmed = redisTemplate.opsForStream().trim(REMOTE_STREAM_KEY, emergencyRetainCount);
