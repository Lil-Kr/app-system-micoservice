...
import org.cy.micoservice.app.infra.console.dao.permission.SysDictMapper;
...
public class SysDictServiceImpl extends ServiceImpl<SysDictMapper, SysDict> implements SysDictService {
	...
	public ApiResp<String> add(DictSaveReq req) {
		...
		if (insert >= 1) {
			permissionCacheService.updateDictCache(dict.getSurrogateId(), dict, CommonConstants.BUS_CREATE);
	...
	public ApiResp<String> edit(DictSaveReq req) {
		...
		try {
			...
			if (update >= 1) {
				permissionCacheService.updateDictCache(after.getSurrogateId(), after, CommonConstants.BUS_EDIT);
	...
	public ApiResp<String> delete(Long surrogateId) {
		...
		if (delete >= 1) {
			// 移除缓存
			permissionCacheService.removeDicCache(surrogateId);
			return ApiResp.success();
		}
